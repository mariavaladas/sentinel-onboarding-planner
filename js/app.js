...
import { initGanttPlanner } from './gantt-planner.js?v=16';
import { renderMitreCoverage } from './modules/mitre.js?v=16';
import { handleNlpInput, handleNlpKeydown } from './modules/search.js?v=16';
import {
    ...
    toggleVendor
} from './modules/solutions.js?v=16';
import { exportTopologyAsPdf, exportTopologyAsPng, renderTopology } from './modules/topology.js?v=16';
...
// Query the workspace via ARM-proxied Log Analytics query API
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                lastError = new Error(`Workspace query failed (${response.status}): ${details}`);
                continue; // retry on transient failures (429, 500, 503)
            }
            const result = await response.json();
            if (returnRows) {
                const table = result?.tables?.[0];
                return { columns: (table?.columns || []).map(c => c.name || c), rows: table?.rows || [] };
            }
...
function logWorkspaceConnectorSummary(summary = {}) {
    ...
    const summaryLines = summary.usedTableFallback
        ...
        : [
            `[Sentinel Planner] Connector Summary:\n  - API returned: ${summary.apiConnectorCount || 0} connector resource(s) (${summary.uniqueApiConnectorCount || 0} unique)\n  - Mapped to solutions: ${summary.mappedConnectorCount || 0} connector(s) (covering ${summary.mappedSolutionIdCount || 0} solution ID(s))\n  - Unmatched (shown as "Other"): ${summary.unmatchedConnectorCount || 0}\n  - Total shown in topology: ${summary.totalShownInTopology || 0}\n  - Active confidence signals: ${summary.activeApiConnectorCount || 0}`
        ];

    console.info(summaryLines[0]);
}

function buildWorkspaceConnectorStatusMessage(workspaceName, summary = {}, { reconnected = false } = {}) {
    const prefix = `${reconnected ? 'Reconnected' : 'Connected'} to ${workspaceName}.`;
    const connectorCount = Number(summary?.discoveryConnectorCount) || 0;

    if (summary?.usedTableFallback) {
        if (connectorCount > 0) {
            return `${prefix} ${connectorCount} connected data connector${connectorCount === 1 ? '' : 's'} detected from active tables.`;
        }
        return `${prefix} No connected data connectors detected.`;
    }

    if (connectorCount <= 0) {
        return `${prefix} No connected data connectors found.`;
    }

    return `${prefix} ${connectorCount} connected data connector${connectorCount === 1 ? '' : 's'} found.`;
}
...
async function onWorkspaceChange() {
    ...
    try {
        // Strategy: trust Sentinel dataConnector resources first.
        // 1. dataConnectors API (2024-09-01) — source of truth for configured workspace connectors
        // 2. Workspace query — heuristic table scan used only when the API returns zero connectors
        // 3. Fallback: Tables ARM API (GET) — lists provisioned tables when the query fails
        const [connectors, activeDataTypes] = await Promise.all([
            ...
            queryWorkspace(subscriptionId, resourceGroupName, workspaceName,
                'Usage | where TimeGenerated > ago(14d) | summarize TotalMB=sum(Quantity), LastLog=max(TimeGenerated) by DataType | where TotalMB > 0 | project DataType, LastLog',
                ...
                try {
                    ...
                    return { data: [], failed: true, reason: err.message };
                }
            })
        ]);
        ...
        if (usageQueryFailed) {
            ...
            // Multi-column KQL result: { columns, rows: [[DataType, LastLog], ...] }
            ...
            // Fallback: plain array of DataType strings (Tables ARM API or legacy)
        ...
        if (usedTableFallback) {
            ...
            const supplementCount = discoveryConnectors.length - uniqueApiConnectors.length;
            console.info(`[Sentinel Planner] Workspace discovery: ${uniqueApiConnectors.length} connector resource(s) from API + ${supplementCount} additional connector(s) from active tables = ${discoveryConnectors.length} total. Active confidence signals: ${activeConnectorResources.length}.`);
        }
        ...
        const discoveryResult = setConnectedSolutionsFromWorkspace(discoveryConnectors, {
            ...
            additionalTableConnectorCount: 0,
        ...
        const workspaceConnectorSummary = discoveryResult.summary || {};
        const statusTone = workspaceConnectorSummary.usedTableFallback ? 'warning' : 'success';
        setWorkspaceStatus(buildWorkspaceConnectorStatusMessage(workspaceName, workspaceConnectorSummary), statusTone);
    } catch (error) {
    ...
    try {
        ...
        const [connectors, activeDataTypes] = await Promise.all([
            ...
            queryWorkspace(subscriptionId, resourceGroupName, workspaceName,
                ...
                if (err.message === WORKSPACE_CONNECTION_EXPIRED_MESSAGE) throw err;
                console.warn('[Sentinel Planner] Auto-reconnect Usage query failed, trying Tables API fallback:', err.message);
                try {
                    const tables = await getWorkspaceTablesViaArm(subscriptionId, resourceGroupName, workspaceName);
                    console.info(`[Sentinel Planner] Tables API fallback returned ${tables.length} tables`);
                    return tables;
        ...
        if (usedTableFallback) {
            ...
            const supplementCount = discoveryConnectors.length - uniqueApiConnectors.length;
            console.info(`[Sentinel Planner] Auto-reconnect: ${uniqueApiConnectors.length} connector resource(s) from API + ${supplementCount} additional connector(s) from active tables = ${discoveryConnectors.length} total. Active confidence signals: ${activeConnectorResources.length}.`);
        }
        ...
        const workspaceConnectorSummary = discoveryResult.summary || {};
        const statusTone = workspaceConnectorSummary.usedTableFallback ? 'warning' : 'success';
        setWorkspaceStatus(buildWorkspaceConnectorStatusMessage(workspaceName, workspaceConnectorSummary, { reconnected: true }), statusTone);
