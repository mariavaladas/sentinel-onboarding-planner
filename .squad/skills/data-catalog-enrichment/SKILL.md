...
5. For operational metadata like onboarding effort and permissions, classify each solution by connector archetype first, then apply per-solution overrides for roles, consent, infrastructure, and notes.
6. Calculate derived values like `priority_score` and `phased_deployment` in code, never by hand.
7. Validate every record after generation: schema presence, foreign-key integrity for dependency links, and score recalculation.

## Validation checklist
- Every solution contains `value_scoring`, `planner`, `export_metadata`, `onboarding`, and `permissions`.
